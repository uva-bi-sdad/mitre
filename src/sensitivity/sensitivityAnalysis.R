# 8/5/19
# Conduct a sensitivity analysis across multiple runs using SMARTscatter.
# Parallelized over a list of simulation study inputs.
# Variables: Home Value, Single Parent Household (y/n), Household Size, Household Income
# Record synthetic output (thinned), error (RMSE for income), total #bins, total #households, and time to run

setwd("~/git/mitrePublic/src/sensitivity")

load("sensitivitySettings.RData")
...
SMARTscatter <- function(
  ...
  rmse <- computeRMSE(syntheticTruth, mcmc_samples, est_vars)
  
  # write .RData file for this iteration
  save(file=paste0("sensitivityRun",ind,".RData"),mcmc_samples,rmse,comptime)
  return(list(mcmc_samples=mcmc_samples,rmse=rmse,comptime=comptime))
}

# test the call to SMARTscatter for settings[[1]]
#test <- SMARTscatter(
#  ind=settings[[1]]$ind,
#  numbgs=settings[[1]]$numbgs,
#  incomebins=settings[[1]]$incomebins,
#  valuebins=settings[[1]]$valuebins,
#  totalbins=settings[[1]]$totalbins,
...
#  syntheticTruth=settings[[1]]$syntheticTruth
#)

# do the full parallel run! (27 iterations)
...
ls <- foreach(k=1:length(settings)) %dopar% {
  SMARTscatter(
    ind=settings[[k]]$ind,
    numbgs=settings[[k]]$numbgs,
    incomebins=settings[[k]]$incomebins,
    valuebins=settings[[k]]$valuebins,
    totalbins=settings[[k]]$totalbins,
    ...
    syntheticTruth=settings[[k]]$syntheticTruth
  )
}
stopCluster(cl)

save.image("sensitivityAllRuns.RData")
